...
- _Crawl Core:_ "FastAPI crawler — fetches any URL (curl_cffi + Playwright fallback), extracts
...
- _JobSherpa:_ "Agentic pipeline — parses a resume + JD, scrapes live interview/salary data
  (Glassdoor, AmbitionBox, Levels.fyi), generates a visual PDF with rubrics, ATS scoring, and an
  interview roadmap."
- _Multipart Uploader:_ "CLI from my first internship to learn Go — goroutines, concurrency, and
